...
const serveTodoPage = function(req, res) {
  ...
  if (user) {
    ...
    todoPage = todoPage.replace('__USERNAME__', req.body.username);
    res.send(todoPage);
    return;
  }
  res.redirect('/login.html');
...
const serveSignInPageWithError = function(req, res) {
  let content = readFile('public/signIn.html');
  content = content.replace('hidden', '');
  res.send(content);
};
